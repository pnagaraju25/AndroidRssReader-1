...
public class DbHelper extends SQLiteOpenHelper {
	...
	private static final String SQL_CREATE_FEEDS = String.format(
			"CREATE TABLE %s (%s INTEGER PRIMARY KEY,%s TEXT)", Feeds.TABLE_NAME,
			Feeds.COLUMN_NAME_ID, Feeds.COLUMN_NAME_TITLE);

	private static final String SQL_CREATE_ARTICLES = String.format(
			"CREATE TABLE %s (%s INTEGER PRIMARY KEY,%s TEXT,%s TEXT,%s TEXT,%s INTEGER, %s INTEGER)", Articles.TABLE_NAME,
			Articles.COLUMN_NAME_ID, Articles.COLUMN_NAME_TITLE, Articles.COLUMN_NAME_DESCRIPTION,
	...
	public Article getArticleByTitle(final String title, final boolean fetchParagraphs) {
		...
		Cursor cursor = db.query(Articles.TABLE_NAME, fetchColumns, whereClause, whereClauseArgs, groupBy,
				filterBy, sortOrder);
	...
	private void getArticleParagraphs(final Article article) {
		...
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(Paragraphs.TABLE_NAME, fetchColumns, whereClause, whereClauseArgs, groupBy,
				filterBy, sortOrder);
	...
	public List<Feed> getFeeds() {
		String[] fetchColumns = { Feeds.COLUMN_NAME_ID, Feeds.COLUMN_NAME_TITLE };
		...
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(Feeds.TABLE_NAME, fetchColumns, whereClause, whereClauseArgs, groupBy,
				filterBy, sortOrder);
		...
		while (!cursor.isAfterLast()) {
			...
			feeds.add(feed);
			cursor.moveToNext();
			Log.d("XIMON", "Fetched feed " + feed.getId() + ": " + feed.getTitle());
		}
